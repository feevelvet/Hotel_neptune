...
if (!isset($_GET['pieces']) || !isset($_GET['prix']) || !isset($_GET['personnes'])) {
    $requete = "SELECT * FROM Chambres WHERE Id >= 11";
} else {
    $requete = "SELECT * FROM Chambres WHERE Id >= 11 AND Pieces >= $pieces AND Prix <= $prix AND Personne >= $personnes";
}
// Exécuter la requête
...
<form method="get" action="royale.php">
